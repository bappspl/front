...
class KonserwatorTable extends ModelTable implements ServiceLocatorAwareInterface
{
    ...
    public function deleteKonserwator($ids)
    {
        if(!is_array($ids)) {
            $ids = array($ids);
        }
    ...
    public function save(Konserwator $konserwator)
    {
        $data = array(
            'name' => $konserwator->getName(),
            'status_id'  => $konserwator->getStatusId(),
            'email'  => $konserwator->getEmail(),
            'phone'  => $konserwator->getPhone(),
        );

        $id = (int) $konserwator->getId();
        if ($id == 0) {
            ...
            if ($this->getOneBy(array('id' => $id))) {
                ...
                throw new \Exception('Konserwator id does not exist');
            }
        }
    ...
    public function changeStatusKonserwator($ids, $statusId)
    {
        if(!is_array($ids)) {
            $ids = array($ids);
        }
