...
class PerformanceTable extends ModelTable implements ServiceLocatorAwareInterface
{
    ...
    public function deletePerformance($ids)
    {
        if(!is_array($ids)) {
            $ids = array($ids);
        }
    ...
    public function save(Performance $performance)
    {
        $data = array(
            ...
            'slug' => Inflector::slugify($performance->getName()),
            'status_id'  => $performance->getStatusId(),
            'url'  => $performance->getUrl(),
            'content'  => $performance->getContent(),
            'enter'  => $performance->getEnter(),
            'date'  => $performance->getDate(),
        );

        $id = (int) $performance->getId();
        if ($id == 0) {
            ...
            if ($this->getOneBy(array('id' => $id))) {
                ...
                throw new \Exception('Performance id does not exist');
            }
        }
    ...
    public function changeStatusPerformance($ids, $statusId)
    {
        if(!is_array($ids)) {
            $ids = array($ids);
        }
